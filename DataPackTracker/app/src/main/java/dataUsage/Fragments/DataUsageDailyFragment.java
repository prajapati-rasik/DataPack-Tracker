...
public class DataUsageDailyFragment extends Fragment {
    ...
    //set data in recyleview
    private void setData(View view, PieChart pie, BarChart bar) {
        if (getActivity() != null) {
            if(getArguments() == null) {
                this.asyncTask = new dataUsageAsync(getActivity().getApplicationContext(), view, this.progressBar, makeCalenderInstances.days(7, getActivity().getApplicationContext()), pie, bar);
                this.asyncTask.execute();
            }else{
                this.customAsync = new appUsageAsync(view, getActivity().getApplicationContext(), this.progressBar, makeCalenderInstances.days(7, getActivity().getApplicationContext()),getArguments().getInt("PackageId", 0), pie, bar);
    ...
    public View onCreateView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle bundle) {
        ...
        floatingActionButton.setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
                DataUsageDailyFragment secondView = DataUsageDailyFragment.this;
