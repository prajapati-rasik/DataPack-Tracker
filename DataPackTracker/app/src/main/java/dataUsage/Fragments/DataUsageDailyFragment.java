    ...
    private void setData(View view) {
        if (getActivity() != null) {
            if(getArguments() == null) {
                this.asyncTask = new dataUsageAsync(getActivity().getApplicationContext(), view, this.progressBar, makeCalenderInstances.days(7, getActivity().getApplicationContext()));
                this.asyncTask.execute();
            }else{
                this.customAsync = new dataUsageCustomAsync(view, getActivity().getApplicationContext(), this.progressBar, makeCalenderInstances.days(7, getActivity().getApplicationContext()),getArguments().getInt("PackageId", 0));
