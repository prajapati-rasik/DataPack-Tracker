    ...
    private void setData(View view, PieChart pie, BarChart bar) {
        if (getActivity() != null) {
            if(getArguments() == null) {
                this.asyncTask = new dataUsageAsync(getActivity().getApplicationContext(), view, this.progressBar, makeCalenderInstances.weeks(getActivity().getApplicationContext()), pie, bar);
                this.asyncTask.execute();
            }else{
                this.customAsync = new appUsageAsync(view, getActivity().getApplicationContext(), this.progressBar, makeCalenderInstances.weeks(getActivity().getApplicationContext()), getArguments().getInt("PackageId", 0), pie, bar);
